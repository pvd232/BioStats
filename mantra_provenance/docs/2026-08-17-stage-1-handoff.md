...
- Wrote the first two PyTorch metric functions:
  - [`mse()`](../../metrics/train/mse.py), which validates compatible real
    tensors and returns mean squared error;
  - [`pearson_corr()`](../../metrics/eval/pearson_corr.py), which computes
...
- Added focused model and verifier tests in
  [`test_models_v4.py`](../../tests/test_models_v4.py) and
...
PYTHONDONTWRITEBYTECODE=1 conda run -n mantra python -m py_compile \
  mantra_provenance/models_v4.py mantra_provenance/verifier.py
