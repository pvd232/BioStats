...
[protocol models](../viper/protocol.py) and serialized through the
[canonical document encoder](../viper/serialization.py).
...
repository-relative path and exact bytes. [project_init.py](../viper/project_init.py)
...
sole argument. The implementation lives in [stages.py](../viper/stages.py).
...
[api.py](../viper/api.py) and [application.py](../viper/application.py).
...
[authoring.py](../viper/authoring.py) implements freezing, and
[preflight.py](../viper/preflight.py) implements the host-side metric check.
...
agents respond to a specific failure. [preflight.py](../viper/preflight.py)
...
[stage_execution.py](../viper/stage_execution.py),
[stage_worker.py](../viper/stage_worker.py), and
[runtime.py](../viper/runtime.py).
...
lives in [materialization.py](../viper/materialization.py).
...
credential, response, and body-verification contract. [http.py](../viper/http.py)
...
created. [resume.py](../viper/resume.py) implements capture, safe loading, and
restoration.
...
recomputed value with the recorded measurement. [metrics.py](../viper/metrics.py)
and [metric_execution.py](../viper/metric_execution.py) implement those paths.
...
to independently recomputed metric values. [benchmark.py](../viper/benchmark.py)
constructs the result; `verify_benchmark_result()` in
[verifier.py](../viper/verifier.py) checks it.
...
storage location, byte count, and digest. [local_store.py](../viper/local_store.py),
[journal.py](../viper/journal.py), and [workspace.py](../viper/workspace.py)
...
contents support a later claim. [verify_run_result()](../viper/verifier.py)
...
verified run. Their Python interfaces live in [inspection.py](../viper/inspection.py),
and their stable application surface is defined in
[application.py](../viper/application.py).
...
5. VIPER, [package release candidate](releases/0.1.0a1.md), commit and executed
   acceptance evidence.
6. VIPER source, [protocol models](../viper/protocol.py), authored and resolved
   document definitions.
7. VIPER source, [plan authoring](../viper/authoring.py), canonical freezing and
   source-identity checks.
8. VIPER source, [stage interface](../viper/stages.py) and
   [stage execution](../viper/stage_execution.py), typed invocation and worker
   launch.
9. VIPER source, [project initialization](../viper/project_init.py), generated
   user-project source and tests.
10. VIPER source, [complete-plan runner](../viper/runner.py) and
   [verifier](../viper/verifier.py), attempt execution, publication, and
   verification.
