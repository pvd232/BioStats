...
Preview a migration:
    ./tools/migrate_codex_project_ref.py --old "/old/path" --new "/new/path" --dry-run

Run a migration:
    ./tools/migrate_codex_project_ref.py --old "/old/path" --new "/new/path"
"""
...
class SqliteUpdate:
    """Describe one exact SQLite column update."""
...
def default_codex_home() -> Path:
    """Return the configured Codex state directory."""
    return Path(os.environ.get("CODEX_HOME", Path.home() / ".codex")).expanduser()


def default_sqlite_home(codex_home: Path) -> Path:
    """Return the directory containing Codex auxiliary databases."""
    return Path(os.environ.get("CODEX_SQLITE_HOME", codex_home / "sqlite")).expanduser()


def timestamp() -> str:
    """Return a filesystem-safe timestamp for backup directories."""
    return datetime.now().strftime("%Y%m%d-%H%M%S")


def backup_file(path: Path, backup_dir: Path, dry_run: bool) -> Path | None:
    """Copy one state file into the migration backup directory."""
    if not path.exists():
        return None
...
def sqlite_count(db_path: Path, table: str, column: str, value: str) -> int:
    """Count database rows whose selected column equals one value."""
...
def matching_rollout_paths(state_db: Path, old_path: str) -> list[Path]:
    """Find rollout logs belonging to threads rooted at the old path."""
...
def sqlite_replace_exact(update: SqliteUpdate, new_path: str, dry_run: bool) -> int:
    """Apply one exact path replacement and return the affected row count."""
    ...
    with sqlite3.connect(update.db_path) as conn:
        ...
        conn.commit()
        print(
            f"Updated {count} row(s) in {update.db_path}:{update.table}.{update.column}"
        )
        return int(count)
...
def update_rollout_session_meta(
    ...
    """Replace the project path in rollout session metadata."""
    changed = 0
    for rollout_path in rollout_paths:
        ...
        try:
            ...
            print(
                f"Skipping non-UTF-8 rollout log {rollout_path}: {exc}",
                file=sys.stderr,
            )
            continue
        ...
        try:
            ...
            print(
                f"Skipping rollout log with invalid first event {rollout_path}: {exc}",
                file=sys.stderr,
            )
            continue
        ...
        lines[0] = (
            json.dumps(first_event, ensure_ascii=False, separators=(",", ":")) + "\n"
        )
...
def update_preferences_last_directory(
    ...
    """Replace the old path in the desktop file-picker preference."""
    ...
    try:
        ...
        print(
            f"Skipping preferences file; it is not valid JSON: {exc}",
            file=sys.stderr,
        )
        return False
    ...
    print(f"Updated selectfile.last_directory: {last_directory} -> {updated_directory}")
    return True


def path_arg(value: str) -> str:
    """Expand a command-line path without requiring it to exist."""
    return str(Path(value).expanduser())


def parse_args() -> argparse.Namespace:
    """Parse migration source, target, safety, and storage arguments."""
...
def main() -> int:
    """Run the guarded local Codex project-reference migration."""
    args = parse_args()
    ...
    if not new_path_obj.exists():
        if args.create_target:
            ...
            print(
                ...
                "Move or create it first, or use --create-target or "
                "--allow-missing.",
                file=sys.stderr,
            )
            return 2
    ...
    dbs_to_backup = sorted(
        {update.db_path for update in updates if update.db_path.exists()}
    )
